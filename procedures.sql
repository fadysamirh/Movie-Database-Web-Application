...
SET @type=0
ELSE IF (@username IN (SELECT username FROM Vendor))
SET @type=1
...
@serial INT
AS
IF((@customername IN (SELECT username FROM Customer)) AND EXISTS(SELECT * FROM Product WHERE serial_no=@serial and available='1'))
...
CREATE PROC  EditProduct
 ...
 AS
 IF ( @product_name IN (select product_name from Product ) and (@vendorname IN (select username from vendor where activated = 1) ) and EXISTS(
...
IF ((@vendorname not IN (select username from Vendor where activated = 1)) or (@offerid not in (select offer_id from offer )) or @serial not in (select serial_no from Product )and EXISTS(
 SELECT * FROM Vendor V INNER JOIN Product P ON V.username=P.vendor_username
...
else print('the expiry date has already past')
GO
----------
